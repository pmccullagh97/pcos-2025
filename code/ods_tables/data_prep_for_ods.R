...
# Create table
table_3.1aa_data <- setNames(
  data.frame(
    counts[[1]],   # Response labels
    counts[[3]],   # Percent values
    check.names = FALSE
  ),
...
# Create new row
new_row <- as.list(rep(NA, ncol(table_3.1aa_data)))
names(new_row) <- names(table_3.1aa_data)
...
# Append to dataframe
table_3.1aa_data[,1] <- as.character(table_3.1aa_data[,1])
table_3.1aa_data <- rbind(table_3.1aa_data, new_row)


# Reasons for distrusting NISRA
table_3.1ab_data <- setNames(
  data.frame(
    counts2[[1]],
    counts2[[3]],
    check.names = FALSE
...
# Create new row
new_row2 <- as.list(rep(NA, ncol(table_3.1ab_data)))
names(new_row2) <- names(table_3.1ab_data)
...
# Append to dataframe
table_3.1ab_data[,1] <- as.character(table_3.1ab_data[,1])
table_3.1ab_data <- rbind(table_3.1ab_data, new_row2)
